...
(function (global) {
  "use strict";
  console.log("Loading WK Framework");
  ...
  const supported_modules = {
    Apiv2: {
      url: "https://greasyfork.org/scripts/38581-wanikani-open-framework-apiv2-module/code/Wanikani%20Open%20Framework%20-%20Apiv2%20module.js?version=1180673",
    },
    ItemData: {
      url: "https://greasyfork.org/scripts/38580-wanikani-open-framework-itemdata-module/code/Wanikani%20Open%20Framework%20-%20ItemData%20module.js?version=1187212",
    },
    Jquery: {
      url: "https://greasyfork.org/scripts/451078-wanikani-open-framework-jquery-module/code/Wanikani%20Open%20Framework%20-%20Jquery%20module.js?version=1091794",
    },
    Menu: {
      url: "https://greasyfork.org/scripts/38578-wanikani-open-framework-menu-module/code/Wanikani%20Open%20Framework%20-%20Menu%20module.js?version=1239374",
    },
    Progress: {
      url: "https://greasyfork.org/scripts/38577-wanikani-open-framework-progress-module/code/Wanikani%20Open%20Framework%20-%20Progress%20module.js?version=1091792",
    },
    Settings: {
      url: "https://greasyfork.org/scripts/38576-wanikani-open-framework-settings-module/code/Wanikani%20Open%20Framework%20-%20Settings%20module.js?version=1091793",
    },
  };
  ...
  published_interface.support_files = {
    ...
    "jqui_wkmain.css":
      "https://raw.githubusercontent.com/rfindley/wanikani-open-framework/1550af8383ec28ad406cf401aee2de4c52446f6c/jqui-wkmain.css",
  };
  ...
  function include(module_list) {
    ...
    let loaded = [],
      failed = [];
    let no_cache = split_list(
      localStorage.getItem("wkof.include.nocache") || "",
    );
    for (let idx = 0; idx < module_names.length; idx++) {
      ...
      let use_cache =
        no_cache.indexOf(module_name) < 0 && no_cache.indexOf("*") < 0;
      if (!use_cache) file_cache_delete(module.url);
      if (await_load === undefined)
        include_promises[module_name] = await_load = load_script(
          module.url,
          use_cache,
        );
    ...
    function check_done() {
      ...
      if (failed.length === 0)
        include_promise.resolve({ loaded: loaded, failed: failed });
      else
        include_promise.reject({
          error: "Failure loading module",
          loaded: loaded,
          failed: failed,
        });
    }
  ...
  function load_file(url, use_cache) {
    ...
    let no_cache = split_list(
      localStorage.getItem("wkof.load_file.nocache") || "",
    );
    ...
    function process_result(event) {
      ...
      if (event.target.status >= 400 || event.target.status === 0)
        return fetch_promise.reject(event.target.status);
      if (use_cache) {
        file_cache_save(url, event.target.response).then(
          fetch_promise.resolve.bind(null, event.target.response),
        );
      } else {
  ...
  function wait_state(state_var, value, callback, persistent) {
    ...
    if (state_listeners[state_var] === undefined)
      state_listeners[state_var] = [];
    persistent = persistent === true;
    let current_value = state_values[state_var];
    if (persistent || value !== current_value)
      state_listeners[state_var].push({
        callback: callback,
  ...
  function handle_page_events(event_name, event) {
    ...
    page_handlers.forEach((handler) => {
      ...
      if (typeof handler[event_name] === "function")
  ...
  function file_cache_save(name, content, extra_attribs) {
    ...
    function save(db) {
      ...
      wkof.file_cache.dir[name] = Object.assign(
        { added: now, last_loaded: now },
        extra_attribs,
      );
      file_cache_dir_save(true /* immediately */);
  ...
  function file_cache_dir_save(immediately) {
    ...
    function save2(db) {
      ...
      store.put({
        name: "[dir]",
        content: JSON.stringify(wkof.file_cache.dir),
      });
    }
  }
  ...
  function file_cache_cleanup() {
    ...
    console.log(
      "Cleaning out " +
        old_files.length +
        ' old file(s) from "wkof.file_cache":',
    );
  ...
  function file_nocache(list) {
    if (list === undefined) {
      ...
      list = list.concat(
        split_list(localStorage.getItem("wkof.load_file.nocache") || ""),
      );
  ...
  function is_turbo_page() {
    return (
      document
        ...
        ?.innerHTML.match("@hotwired/turbo") != null
    );
  }
